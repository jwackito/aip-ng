...
def _make_dataset(date):
    ...
    # Calculate the attacks per destination (Honeypot IPs will be scrambled)
    df = daily[['id.orig_h', 'id.resp_h', 'duration', 'orig_pkts', 'orig_ip_bytes']].groupby(['id.orig_h', 'id.resp_h']).sum()
    df.rename(columns={'duration':'duration', 'orig_pkts':'packets', 'orig_ip_bytes':'bytes'}, inplace=True)
    a = np.array([[orig, dest] for orig, dest in df.index.values])
    df['orig'] = a[:,0]
    df['dest'] = a[:,1]
